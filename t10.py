...
async def get_lighter_account_values():
    """
    Fetches total account value (collateral + unrealized PNL + allocated margins) across all sub-accounts
    using authenticated token for private /account calls.
    
    Hardcodes known account indices from your public query.
    ...
    try:
        # Create SignerClient with correct dict param
        ...
        if err is not None:
            ...
            print("→ Regenerate API key in Lighter app and update .env")
            return
        ...
        # Standard Configuration + access_token
        ...
        async with lighter.ApiClient(configuration=config) as api_client:
            ...
            for index in account_indices:
                ...
                try:
                    ...
                    if '21100' in str(e):
                        print(f"  Access denied for index {index} (API key may only access main account)")
                    else:
                        print(f"  Error for index {index}: {e}")
                    continue
            
            if not accounts_summary:
                print("No accessible accounts. API key issue or no funds/positions.")
                return
