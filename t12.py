...
async def get_lighter_account_values():
    """
    Working script to fetch total account value including positions margins + PNL.
    
    Uses SignerClient directly as ApiClient (handles auth internally).
    Correct response handling: /account returns list of DetailedAccount.
    ...
    try:
        ...
        # Use SignerClient directly as the client for ApiClient - this handles auth automatically
        async with lighter.ApiClient(signer) as api_client:
            ...
            for index in account_indices:
                ...
                try:
                    ...
                    # Returns list of DetailedAccount objects
                    ...
                    if not target_account:
                        print(f"  No data returned for index {index} (access denied or empty)")
                        continue
            ...
            if not accounts_summary:
                ...
                print("Your API key may have read permissions only for certain accounts, or accounts are empty.")
                return
