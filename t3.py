...
async def get_lighter_account_values(l1_address: str = None):
    ...
    using authenticated read-only auth tokens.
    
    Requires in .env:
    API_KEY_PRIVATE_KEY=0x... (your API private key)
    API_KEY_INDEX=3 (your API key index)
    MAIN_ACCOUNT_INDEX=185609 (your main account index)
    ...
    try:
        # Create SignerClient for auth token generation only
        ...
        if err is not None:
            ...
            print("Verify API_KEY_PRIVATE_KEY, API_KEY_INDEX, MAIN_ACCOUNT_INDEX")
            return
        ...
        config.access_token = auth_token  # This adds Authorization header
        
        async with lighter.ApiClient(config) as api_client:
            account_api = lighter.AccountApi(api_client)
            
            # Get account list (public)
