...
async def get_lighter_account_values():
    """
    Final corrected script that matches your working bot exactly.
    
    The key fix: positions is a list of AccountPosition objects, NOT a dict.
    We iterate over the list and sum the fields.
        ...
        async with lighter.ApiClient(configuration=config) as api_client:
            ...
            for index in account_indices:
                ...
                try:
                    ...
                    # Critical fix: positions is a LIST of AccountPosition objects
                    positions_list = getattr(target_account, 'positions', []) or []
                    ...
                    for pos in positions_list:
                        ...
                        realized_pnl += float(getattr(pos, 'realized_pnl', 0) or 0)
                        allocated_margin += float(getattr(pos, 'allocated_margin', 0) or 0)
