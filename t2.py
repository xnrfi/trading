...
async def get_lighter_account_values(l1_address: str = None):
    ...
    MAIN_ACCOUNT_INDEX=185609  # Your main account index (from previous output)
    ...
    main_account_index = int(os.getenv("MAIN_ACCOUNT_INDEX", "0"))  # Required for signer
    ...
    if main_account_index == 0:
        print("ERROR: MAIN_ACCOUNT_INDEX not set in .env")
        print("   Use your main account index (e.g. 185609 from your earlier output)")
        return
    ...
    try:
        # Create authenticated SignerClient
        # Correct parameters based on current SDK: private_key, api_key_index, account_index
        ...
        if err is not None:
            ...
            print("Check your API_KEY_PRIVATE_KEY, API_KEY_INDEX, MAIN_ACCOUNT_INDEX, and permissions")
            return
        ...
        async with lighter.ApiClient(client) as api_client:
            ...
            # Get ALL account indices for this L1 address (public endpoint)
            ...
            # Fetch details for EACH account (requires auth)
